...
export async function PUT(
  ...
  const { name, species,breed,gender,target_weight, birthdate, chip_number, photo_url } = await req.json()

  const updated = await prisma.pet.update({
    where: { id: BigInt(id) },
    data: { name, species,breed,gender, birthdate: new Date(birthdate), chip_number, target_weight,photo_url },
  })
